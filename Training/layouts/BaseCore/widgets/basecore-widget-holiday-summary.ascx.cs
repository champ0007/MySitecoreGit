    ...
    public partial class WidgetHolidaySummary : BaseWidget
    {
        private void Page_Load(object sender, EventArgs e)
        {
            ...
            if (source.TemplateID == TemplateReferences.Holiday)
            {
                ...
                txtType.Item = FieldUtils.GetReferenceFieldItem("Type");
                txtDifficulty.Item = FieldUtils.GetReferenceFieldItem("Difficulty");

                List<Item> terrains = FieldUtils.GetListFieldItems("Terrain");

                if (terrains.Any())
                {
                    litTerrain.Text = string.Join(", ", terrains.Select(x => FieldUtils.GetFieldValue(x, "Text")));
                }
            }
        }      
    }
